...
(function() {
  var Puzzle, Sliding, Square, arraySwap, countInversions, getHeight, getLeft, getPosX, getPosY, getTop, getWidth, int, runOnceAt, slowlyMove, solvable, swap;
  ...
  Sliding = (function() {
    ...
    arg2pos = function(pos) {
      if (arguments.length > 1) {
        pos = [arguments[0], arguments[1]];
      } else {
        pos = this.position[pos] != null ? this.position[pos] : pos;
      }
      return pos;
    };
    ...
    function Sliding(rows, cols, emptyPos) {
      ...
      this.originalEmtpyPos = [emptyPos[0], emptyPos[1]];
      this.grid = [];
      this.position = [];
      this.position[0] = [this.emptyPos[0], this.emptyPos[1]];
      id = 1;
      for (row = _i = 0, _ref2 = rows - 1; 0 <= _ref2 ? _i <= _ref2 : _i >= _ref2; row = 0 <= _ref2 ? ++_i : --_i) {
        ...
        for (col = _j = 0, _ref3 = cols - 1; 0 <= _ref3 ? _j <= _ref3 : _j >= _ref3; col = 0 <= _ref3 ? ++_j : --_j) {
          if (row === this.emptyPos[0] && col === this.emptyPos[1]) {
            this.grid[row][col] = 0;
          } else {
    ...
    Sliding.prototype.solvable = function(pos) {
      return solvable(this.squares, this.rows, this.cols, this.emptyRow);
    };
    ...
    Sliding.prototype.shuffle = function() {
      var last, rand, _i, _ref;
      for (last = _i = _ref = this.rows * this.cols - 1; _ref <= 0 ? _i <= 0 : _i >= 0; last = _ref <= 0 ? ++_i : --_i) {
        rand = Math.floor(Math.random() * (last + 1));
        this.swap(last, rand);
      }
      if (!this.solvable()) {
    ...
    Sliding.prototype.swap = function(p1, p2) {
      ...
      this.grid[y1][x1] = id2;
      this.grid[y2][x2] = id1;
      this.position[id1] = p2;
      if (id1 === 0) {
        this.emptyPos = p2;
      }
      this.position[id2] = p1;
      if (id2 === 0) {
        this.emptyPos = p1;
      }
      return this;
    ...
    Sliding.prototype.slide = function(pos) {
      var col, row, _i, _j, _ref, _ref1, _ref2, _ref3;
      pos = arg2pos.apply(this, arguments);
      if (!this.slidable(pos)) {
        return this;
      }
      if (pos[0] === this.emptyPos[0]) {
        for (col = _i = _ref = this.emptyPos[1], _ref1 = pos[1]; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; col = _ref <= _ref1 ? ++_i : --_i) {
          this.swap([pos[0], col], this.emptyPos);
        }
      } else if (pos[1] === this.emptyPos[1]) {
        for (row = _j = _ref2 = this.emptyPos[0], _ref3 = pos[0]; _ref2 <= _ref3 ? _j <= _ref3 : _j >= _ref3; row = _ref2 <= _ref3 ? ++_j : --_j) {
          this.swap([row, pos[1]], this.emptyPos);
        }
      }
      return this;
    };

    Sliding.prototype.slidable = function(pos) {
      var _ref, _ref1;
      pos = arg2pos.apply(this, arguments);
      return ((0 <= (_ref = pos[0]) && _ref < this.rows) && (0 <= (_ref1 = pos[1]) && _ref1 < this.cols)) && !(pos[0] === this.emptyPos[0] && pos[1] === this.emptyPos[1]) && (pos[0] === this.emptyPos[0] || pos[1] === this.emptyPos[1]);
    };
